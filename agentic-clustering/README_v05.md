...
flowchart TD
    ...
    Standardize --> KMeans[KMeans初回実行<br/>k=2-28探索]
    KMeans --> EvalKMeans{品質評価<br/>総合スコア60以上?}
    
    EvalKMeans -->|Yes| PCA1[PCA実行]
    EvalKMeans -->|No| AltClustering[代替クラスタリング試行]
    
    AltClustering --> DBSCAN[DBSCAN実行<br/>eps/min_samples探索]
    DBSCAN --> EvalDBSCAN{DBSCAN評価}
    
    EvalDBSCAN --> CheckClusters{クラスタ数50以下?}
    CheckClusters -->|No| TriggerHDBSCAN[HDBSCAN自動起動<br/>目標50クラスタ]
    CheckClusters -->|Yes| CompareAll[手法比較]
    TriggerHDBSCAN --> HDBSCAN[HDBSCAN実行<br/>min_cluster_size探索]
    
    HDBSCAN --> CompareAll
    CompareAll --> FilterDBSCAN{DBSCANクラスタ数50超?}
